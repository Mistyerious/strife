...
export class WebsocketShard extends EventEmitter {
    ...
    private send(data: IWebsocketShardData) {
        if(!this.connection) { return Promise.reject(new Error('Not Connected to the gateway')) }
        if(!Util.isValidRequest(data)) { return Promise.reject(new Error('Invalid Request')) }
