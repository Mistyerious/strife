...
import {IWebsocketShardData} from "./Constants";
...
export class Util {
    ...
    static isValidPresence(obj){
        ...
        if(!['online', 'dnd', 'idle', 'offline'].includes(obj.status = obj.status.toLowerCase())) { return false }
        if(!isArray(obj.activities)) { return false }
